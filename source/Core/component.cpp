...
MStatus Mithrig::System::link(const MObject& node, const MString& attr, MDGModifier* dg_mod)
{
	return DataNode::link(node, m_datanode, attr, dg_mod);
}


MObject Mithrig::System::get_link(const MString& attr, unsigned int index)
{
	return DataNode::get_link(m_datanode, attr, index);
}


MObjectArray Mithrig::System::get_links(const MString& attr)
{
